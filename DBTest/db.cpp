...
int do_semantic(token_list *tok_list)
{
  ...
  token_list *cur = tok_list;
			...
			((cur->next != NULL) && (cur->next->tok_value == K_TABLE)))
	{
		...
		// // new file call to <table_name>.tab 
		// p = fopen("table_name.tab", "wbc"); 
		// if((fhandle = p) == NULL)
		// {
		// 	rc = FILE_OPEN_ERROR;
		// }
		...
		// else
		// {
		// 	tab_entry.record_offset = sizeof(tpd_entry);
		// 			new_entry = (tpd_entry*)calloc(1, tab_entry.record_size);
		// }
					...
					((cur->next != NULL) && (cur->next->tok_value == K_TABLE)))
	{
		...
		cur_cmd = DROP_TABLE;
		// delete table_name.tab when completed 
		// fclose(p);
		// remove("table_name.tab");
  ...
  {
		printf("Invalid statement-Ln398\n");
		rc = cur_cmd;
	}
              ...
              /* make column name case sensitive */
				...
				if (!rc)
				{
					...
					else
					{
						...
						/*
						//build table file header 
						while (header->record_size % 4 != 0){
							header->record_size++;
						}
						header->record_offset = sizeof(table_file_header);
						header->file_size = sizeof(table_file_header);
						char *addTableName = (char*)malloc(sizeof(tab_entry.table_name) + 4);
						strcat(addTableName, tab_entry.table_name); 
						strcat(addTableName, ".tab");	
						printf("Created New File: %s\n", addTableName);	
						// calculate record size 
						
						printf("record_size: %d\n", record_size);	
						// create file with new table name 
						std::ofstream outputFile(addTableName);
						if ((fp = fopen(addTableName, "wbc")) == NULL)	
						{
							rc = FILE_OPEN_ERROR; 
						}
						else
						{
							...
							fwrite(&record_offset, sizeof(int), 1, fp);
						...
						*/
						...
						// 		case T_CHAR:
						// 		case T_VARCHAR: 
						// 			header->record_size += 1 + col_entry->col_len;
						// 		break;
						// 	}
						...
						//  char *addTableName = (char*)malloc(sizeof(new_entry->table_name) + 4);
						//  strcat(addTableName, new_entry->table_name); 
						//  strcat(addTableName, ".tab");	
						...
						// 	rc = FILE_OPEN_ERROR; 
						// }
						// fwrite((void*) header, sizeof(table_file_header), 1, fp); 
						// if (ferror(fp))
						// {
						// 	remove(new_entry->table_name);
						// 	printf("hi\n"); 
						...
						// 	memset(buffer, '\0', (size_t) header->record_size);
						// 	memcpy(buffer, NULL, (size_t) header->record_size);
						// 	fwrite(buffer, (size_t) header->record_size, 1, fp);	
						// }
...
int sem_drop_table(token_list *t_list)
{
	...
	else
	{
		...
		else
		{
			...
			{
				char *deleteTableName = (char*)malloc(sizeof(tab_entry->table_name) + 4);
				strcat(deleteTableName,tab_entry->table_name);
				strcat(deleteTableName, ".tab");
				printf("Delete file: %s\n:", deleteTableName);
				if (remove(deleteTableName) == 0)
				{
					printf("Removed File\n");
				}
				else 
		...
		else  
	   {
		...
		else 
		{
			cur = cur->next;
				...
				char** records;
				records = get_records(tab_entry, header);
...
char** get_records(tpd_entry* tab_entry, table_file_header* t_header)
{
	FILE* fhandle;
	char **records; 
   records = (char**)calloc(sizeof(char**), 100); //100 records for space
	char* filename = (char*)malloc(sizeof(tab_entry->table_name) + 5);
	
	//get_file_name(tab_entry->table_name, filename);
	strcpy(filename, tab_entry->table_name);
	strcat(filename, ".tab");
	...
	for (int i = 0; i < 100; i++)
	{
		records[i] = (char*)calloc((size_t)t_header->record_size, 1);
	}

	if (t_header->num_records == 0)
	{
		return records; 
	}

	unsigned char buffer[t_header->record_size];
	memset(buffer, '\0', t_header->record_size);
	fseek(fhandle, t_header->record_offset, SEEK_SET);
	for (int i = 0; i < t_header->num_records; i++)
	{
		fread((void*) buffer, (size_t) t_header->record_size, 1, fhandle); 
		memcpy((void*) records[i], (void*) buffer, t_header->record_size);
	}
	fclose(fhandle);
...
int sem_select(token_list *t_list)
{
	...
	else 
	{
		...
		{ 
			...
			table_file_header* header = load_file_header(tab_entry->table_name);
			// char *filename = (char*)malloc(sizeof(tab_entry->table_name) + 4);
			// strcat(filename, tab_entry->table_name);
			// strcat(filename, ".tab");
			// if ((fp1 = fopen(filename, "rbc")) == NULL)
			// {
			// 	printf("uable to read file\n");
			// 	return NULL;
			// }
			...
			//print columns 
			// cd_entry* columns = (cd_entry*) calloc(tab_entry->num_columns, sizeof(cd_entry));
			// fseek(fp1, sizeof(tpd_list), SEEK_SET); 

			cd_entry* columns = get_columns(tab_entry);
			if (projection)
			{
				p_columns = get_projected_columns(columns, column_names, tab_entry->num_columns, &p_record_size, &num_pcols);
			}
			char** records = get_records(tab_entry, header);
			cur = cur->next;
			if (!projection)
			{
...
https://sjsu.instructure.com/conversations  /* Open for read */
